...
### Context
- **Spec**: Integrate Lemon Squeezy for payment processing
- **Stack**: Next.js + Supabase + Prisma
- **Constitution**: Server Actions, webhook verification, MoR for tax handling
...
Integrate Lemon Squeezy for subscriptions and one-time payments. Lemon Squeezy acts as Merchant of Record, handling global tax/VAT. Support checkout flow via hosted checkout and webhooks for payment events.

## Security Considerations

**Authentication:**
- Lemon Squeezy API key stored in environment variable (never committed)
- Webhook signature verification using x-signature header

**Authorization:**
- Only authenticated users can initiate checkout
- Users can only view their own orders/subscriptions
- Customer portal for self-service billing management

**Data Protection:**
- Never store payment details (handled by Lemon Squeezy as MoR)
- Store only Lemon Squeezy order_id and customer_id
- Log payment attempts with PII redacted

**Webhooks:**
- Verify webhook signature before processing
- Idempotent event handling (track processed order_ids)
- Handle key events: order_created, subscription_created, subscription_updated
...
**Client Errors (4xx):**
- 400 Bad Request: Invalid checkout data
- 401 Unauthorized: Missing/invalid API key
- 404 Not Found: Order/subscription not found

**Server Errors (5xx):**
- 500 Internal: Lemon Squeezy API error
- 503 Unavailable: Lemon Squeezy API down (retry with backoff)

**Recovery Strategy:**

**Payment failed at checkout:**
- User message: "Payment was not completed. Please try again."
- Log: Checkout attempt, user ID
- Recovery: User retries checkout

**Lemon Squeezy API timeout:**
- User message: "Processing your request. Please wait."
- Log: Timeout, request details
- Recovery: Webhook will confirm order when complete
```
...
**Question:** How to handle duplicate webhook events (Lemon Squeezy may retry)?

**Options Considered:**

**Option A: Track processed order_ids in database**
- Pros: Simple, reliable deduplication
- Cons: Database lookup on every webhook
- Complexity: Low

**Option B: Request deduplication cache (Redis/Upstash)**
- Pros: Fast, prevents backend work
- Cons: Cache miss still allows duplicates
- Complexity: Medium

**Decision:** Track processed order_ids in database
...
**Trade-offs:**
- Simple database check for idempotency
- Acceptable: Low volume, database lookup is fast
```
