...
**Suggested after**:
- `/lite-spec` - Spec exists, ready to visualize UI
...
- `.shipkit-lite/specs/active/[feature].md` - Feature spec provides context
  - **Workflow:** Create spec first with `/lite-spec`, then prototype from it
...
- `.shipkit-lite/why.md` - Vision informs design choices (brand, audience)
...
```bash
ls .shipkit-lite/specs/active/*.md 2>/dev/null
```

**If NO spec found:**
```
⚠️  No spec found in .shipkit-lite/specs/active/

Recommended: Create a spec first with /lite-spec
- Spec clarifies requirements before exploring UI
- Prototyping without spec risks building wrong solution

Options:
1. Create spec first (recommended): Run /lite-spec now
2. Proceed without spec: I'll ask questions to understand requirements

Which option? (1/2)
```

**If user chooses "1":** Invoke `/lite-spec` now, then return to prototyping after spec created
...
- `.shipkit-lite/specs/active/[feature].md` - Read spec for requirements
- `.shipkit-lite/why.md` - Understand brand/audience for design choices
...
**Next**: Extract to spec via `/lite-prototype-to-spec` or continue iterating
```
...
2. **Extract to spec** → Run `/lite-prototype-to-spec` to capture UI/UX decisions
3. **Move to implementation** → Prototype stays as reference, proceed to `/lite-plan`
...
**lite-why-project** - Defines project vision
...
**lite-prototype-to-spec** - Extracts prototype learnings into spec
...
**Complements lite-spec** - Not a replacement
...
**Optional (enhances prototyping):**
- `.shipkit-lite/specs/active/[feature].md` - Feature context
- `.shipkit-lite/why.md` - Vision and brand direction
- `.shipkit-lite/implementations.md` - Existing component patterns to reuse
...
**Fix:** Extract to spec via `/lite-prototype-to-spec`, rebuild with `/lite-plan`, delete after shipping
