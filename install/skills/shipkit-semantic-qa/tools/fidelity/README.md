...
| `ssot-checker.py` | Single-source-of-truth risks: a shared metric/field *computed* in >1 file (the `grade_band`x4 pattern from a real-project retrospective). | `{field, fileCount, siteCount, risk, sites[]}[]` |
...
> carries `functionalSurface` — the only corpus with ground truth (the retro codebase) predates
...
(validated against the retro failure shapes); confidence marked per finding.
...
The retro root cause was a **git-timeline property**: surfaces were *built before
...
`_smoke/` mirrors the retro failure shapes, with `_smoke/spec.json` as its
