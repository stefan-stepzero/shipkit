#!/usr/bin/env python3
"""
lite-detect - Unified detection and queue creation
...
def main():
    ...
    print(f"[lite-detect] Running mode: {mode}")
    ...
    try:
        ...
        if not detected:
            print(f"[lite-detect] No items detected for {mode}")
            return 0

        # Create queue
        queue_path = queue_fn(detected)
        print(f"[lite-detect] Created queue: {queue_path}")
        print(f"[lite-detect] Items queued: {len(detected)}")

        return 0

    except FileNotFoundError as e:
        print(f"[lite-detect] File not found: {e}")
        return 1
    except Exception as e:
        print(f"[lite-detect] Error: {e}")
        return 1
