...
description: Orchestrator that manages context loading at session start and routes user requests to appropriate lite skills. Auto-loaded via session-start hook.
---

# lite-shipkit-master - Lightweight Orchestration & Routing

**Purpose**: Session orchestrator that checks file freshness, loads context efficiently, and routes user requests to appropriate lite skills.

**Role**: THE master orchestrator for Shipkit Lite. Loaded automatically at every session start.
...
- `.claude/hooks/lite-session-start.py` exists and loads this skill
- `.claude/skills/lite-shipkit-master/SKILL.md` exists (this file)
- Other lite skills installed in `.claude/skills/lite-*/`

**First-time users**:
- If `.shipkit-lite/stack.md` missing → Suggest `/lite-project-context` to create it
- If no specs/plans exist → Suggest starting with `/lite-spec` or `/lite-product-discovery`
...
3. Load fresh context files:
   ...
   - `.shipkit-lite/architecture.md` (if exists)
   - `.shipkit-lite/why.md` (if exists)
...
|-----------|----------|--------------|
| "Define vision", "Why this project?", "Project goals" | `/lite-why-project` | .shipkit-lite/why.md |
| "Who are our users?", "Create personas", "User research", "User journey" | `/lite-product-discovery` | .shipkit-lite/why.md, stack.md |

### Context & Status

| User Says | Route To | Load Context |
|-----------|----------|--------------|
| "Show status", "Project health", "What's missing?" | `/lite-project-status` | All .shipkit-lite/ (glob scan) |
| "Scan project", "Generate stack", "What's my tech stack?" | `/lite-project-context` | package.json, .env.example |
...
| "Extract prototype", "Prototype to spec", "Capture UI patterns" | `/lite-prototype-to-spec` | .shipkit-mockups/, specs/active/ |
...
| User Says | Route To | Load Context |
|-----------|----------|--------------|
| "Audit UX", "Check UX patterns", "UX gaps" | `/lite-ux-audit` | implementations/ |
| "Create task", "Track TODO", "User tasks" | `/lite-user-instructions` | user-tasks/active.md |
| "Visualize", "HTML report", "Visual communication" | `/lite-communications` | Relevant files based on request |
...
**Prevent manual editing of `.shipkit-lite/` context files:**
...
- `.shipkit-lite/why.md` - Project vision
