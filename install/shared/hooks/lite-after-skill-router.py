#!/usr/bin/env python3
"""
Shipkit Lite - After-Skill Router Hook
...
Flow:
1. Parent skill writes skill name to .shipkit-lite/.last-skill
2. This hook reads that file after any skill completes
3. Routes to lite-detect with appropriate mode
4. lite-detect scans artifacts and creates queue for follow-up skill
"""
...
SKILL_TO_MODE = {
    "lite-spec": "services",           # After spec → detect external services
    "lite-plan": "contracts",          # After plan → detect data structures
}
...
def main():
    ...
    detect_script = claude_dir / 'skills' / 'lite-detect' / 'scripts' / 'detect.py'
    if not detect_script.exists():
        print(f"⚠️  lite-detect script not found at {detect_script}")
        return 0
